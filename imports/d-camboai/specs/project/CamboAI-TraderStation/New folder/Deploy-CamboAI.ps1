...
function Backup-ExistingData {
    ...
    try {
        docker-compose -f docker-compose.prod.yml exec -T postgres pg_dump -U cambo_user cambo_ai_trader > "$backupDir\database_backup.sql" 2>$null
...
function Deploy-Application {
    ...
    do {
        ...
        $dbReady = docker-compose -f docker-compose.prod.yml exec -T postgres pg_isready -U cambo_user 2>$null
    ...
    docker-compose -f docker-compose.prod.yml exec -T postgres psql -U postgres -f /docker-entrypoint-initdb.d/init-db.sql 2>$null
...
function Test-Deployment {
    ...
    try {
        $dbTest = docker-compose -f docker-compose.prod.yml exec -T postgres psql -U cambo_user -d cambo_ai_trader -c "SELECT 1;" 2>$null
        if ($dbTest -match "1") {
