...
$UvicornCmd = if (Test-Path $UvicornExe) { $UvicornExe } else { $null }
...
Ensure-Dependency -ModuleName 'uvicorn' -InstallName 'uvicorn'
Ensure-Dependency -ModuleName 'fastapi' -InstallName 'fastapi'
...
try {
  ...
  if (-not $UvicornCmd){
    Start-Process -FilePath $VenvPython -ArgumentList "-m","uvicorn","simple_server:app","--host","0.0.0.0","--port","$Port" -RedirectStandardOutput $outLog -RedirectStandardError $errLog -WindowStyle Hidden | Out-Null
  } else {
    Start-Process -FilePath $UvicornCmd -ArgumentList "simple_server:app","--host","0.0.0.0","--port","$Port" -RedirectStandardOutput $outLog -RedirectStandardError $errLog -WindowStyle Hidden | Out-Null
  }
} finally {
  Pop-Location
}
